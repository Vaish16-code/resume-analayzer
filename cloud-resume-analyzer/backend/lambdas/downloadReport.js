...
function buildReport(a) {
  ...
  const found   = JSON.parse(a.found_keywords   || "[]");
  const missing = JSON.parse(a.missing_keywords || "[]");
  const sugg    = JSON.parse(a.suggestions      || "[]");
  const tips    = JSON.parse(a.tips             || "[]");
  ...
  return [
    ...
    "  Powered by AWS Lambda + DynamoDB + S3",
    div,
  ].join("\n");
}
