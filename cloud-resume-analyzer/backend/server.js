...
function buildReport(a) {
  ...
  return [
    div, "       CLOUD ATS RESUME ANALYZER — REPORT", div, "",
    `Resume      : ${a.resumeName}`,
    `Target Role : ${a.selectedRole}`,
    `ATS Score   : ${a.score}%`,
    `Generated   : ${a.createdAt}`,
    `Analysis ID : ${a.analysisId}`,
    "",
    div, "  FOUND KEYWORDS", line,
    a.foundKeywords.length ? a.foundKeywords.map((k) => `  ✔  ${k}`).join("\n") : "  None found.",
    "",
    div, "  MISSING KEYWORDS", line,
    a.missingKeywords.length ? a.missingKeywords.map((k) => `  ✘  ${k}`).join("\n") : "  All keywords matched!",
    "",
    div, "  IMPROVEMENT SUGGESTIONS", line,
    a.suggestions.length ? a.suggestions.map((s, i) => `  ${i+1}. ${s}`).join("\n") : "  No suggestions!",
    "",
    div, "  GENERAL RESUME TIPS", line,
    a.tips.map((t, i) => `  ${i+1}. ${t}`).join("\n"),
    "", div,
    "  Generated by Cloud ATS Resume Analyzer (Local Server)",
    div,
  ].join("\n");
}
...
app.get("/report", (req, res) => {
  try {
    ...
    const text       = buildReport(analysis);
